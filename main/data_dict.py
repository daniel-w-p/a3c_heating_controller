...
class DataDict:
    ...
    def save_data(self, path: str = 'data/table', name: str = 'temp'):
    ...
    def plot_data(self, path: str = 'data/plots', name: str = 'temp'):
        df = pd.DataFrame(self.data)
        ...
        ax1.set_ylim(-25, 50)  # Min - Max temperature

        # Heating On/Off
        ax2 = ax1.twinx()  # additional OY
        ax2.scatter(df['time'], df['heating_on'] * 49 - 24, color='red', label='Bool', marker='o')
        ax2.set_ylabel('Heating on')
        ax2.set_ylim(-25, 50)  # same as Min - Max temperature
